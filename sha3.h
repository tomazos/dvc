...
std::vector<std::byte> SHAKE128(const Collection &input, int outputByteLen) {
  std::vector<std::byte> output(outputByteLen);
...
template <typename Collection>
std::vector<std::byte> SHAKE256(const Collection &input, int outputByteLen) {
  std::vector<std::byte> output(outputByteLen);
...
std::array<std::byte, 224 / 8> SHA3_224(const Collection &input) {
  std::array<std::byte, 224 / 8> output;
...
inline std::string SHA3(std::string_view input) {
  std::string sha;
  sha.resize(32);
  SHA3_256(input.data(), input.size(), sha.data());
  return sha;
}

template <typename Collection>
std::array<std::byte, 256 / 8> SHA3_256(const Collection &input) {
  std::array<std::byte, 256 / 8> output;
...
std::array<std::byte, 384 / 8> SHA3_384(const Collection &input) {
  std::array<std::byte, 384 / 8> output;
...
std::array<std::byte, 512 / 8> SHA3_512(const Collection &input) {
  std::array<std::byte, 512 / 8> output;
...
#define readLane(x, y) (((uint64_t *)state)[ixxx(x, y)])
#define writeLane(x, y, lane) (((uint64_t *)state)[ixxx(x, y)]) = (lane)
#define XORLane(x, y, lane) (((uint64_t *)state)[ixxx(x, y)]) ^= (lane)
#else
#error little endian required
#endif

inline int LFSR86540(uint8_t *LFSR) {
  ...
  for (round = 0; round < 24; round++) {
...

    {
      for (j = 0; j < 7; j++) {
        ...
        if (LFSR86540(&LFSRstate)) XORLane(0, 0, (uint64_t)1 << bitPosition);
      }
    }
  }
}
